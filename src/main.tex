...
\fancyhf{}
\rhead{Saha, Mishra}
\lhead{Algal Blooms in Lake Chapala}
\cfoot{\thepage}
...
We plot the differential equation model~\ref{V3} on a slope field plotter below and observe how it changes $y$, population over $t$, time.
...
\textbf{Parameter $b$:} Since spraying occurs every 15 days, the growth cycle has a period, $T = 15$ days. We also know that cosine function repeats after every $2\pi$ radian. Therefore, for one full cycle to occur in our model, the input of the cosine function increases by $2\pi$ after 1 period, $T$,time.
...
Finally, plugging in the initial condition $y(0)=y_0$ in the general solution~(\ref{3}), we get $C=y_0$. Therefore, the solution is
\begin{equation}
...
Throughout the paper, we developed a differential equation model to describe algal bloom behavior in Lake Chapala. Starting from a simple oscillatory growth idea, we systematically refined the model to ensure biological feasibility and realistic population behavior. By incorporating periodic growth and proportional death as suggested in the reference paper \cite{harwood-algae}, we structure a general model that reflects both seasonal chemical influx (aka. Growth Function) and consistent removal effects (aka. Death Function).

Using the observed experimental data, we calibrated the parameters to reflect spraying frequency, maximum rate of change, and subsequent decrease in growth over time. Following that, we attempt to solve the model which allowed us to analyze how the algae population evolves over time. The solution shows oscillatory behavior with decreasing amplitude, meaning the blooms become less severe as time goes by due to the negative net growth rate.

With given initial population density of $81g/m^2$, the model predicts that the algae population exceeds the odor threshold of $100g/m^2$ during five early time intervals. However, these intervals shorten progressively, and the final exceedance occurs around day 72. After this point, the population remains below the threshold.

Overall, this mathematical model serves as a fairly reasonable and simplified representation of Lake Chapala and, more specifically, the algal bloom behavior in this Lake. It can also serve as an inspiration to show that how differential equations can be used to model environmental systems and interpret real-world data.
