...
\title{Algal Blooms in Lake Chapala: A Differential Equation Model}
\author{Soikat Saha\thanks{Corresponding author. \texttt{sahaso01@gettysburg.edu}} \\
        Shruti D.~Mishra \\
        \small Department of Mathematics, Gettysburg College, \\ 
        \small Gettysburg, PA, USA}
...
\begin{abstract}
Lake Chapala, Mexico's largest freshwater lake, faces recurring algal blooms that affect the quality of drinking water for millions of residents. We develop a differential equation model that captures the algae population dynamics, simulating the impact of periodic nutrient input from agricultural pesticide cycles and proportional removal caused by government intervention and natural loss. The model starts from a deliberately simple oscillatory growth term and no death term at all. However, throughout the model development process, we systematically refine the model through five versions to reflect biological feasibility: non-negative population, population-dependent growth, and proportional death. The resulting model: $dy/dt = a(1-\cos(bt))y - hy$ is then calibrated using published observations. Later, we derive a closed-form solution using separation of variables technique and analyze when population density exceeds an odor threshold of $100~\mathrm{g/m^2}$. We find out that for initial density $81~\mathrm{g/m^2}$, the model predicts five early exceedance windows that shorten over time and blooms remain below threshold after day 72. In general, the work presents how differential equations can model population dynamics in an ecosystem taking into account real-world environmental constraints.
\end{abstract}

\noindent\textbf{Keywords:} algal blooms; Lake Chapala; population dynamics; differential equations; environmental modeling

\section{Introduction}

Lake Chapala in central Mexico has gone beyond just being a mere lake – it supports fishing and farming communities, provides drinking water to millions in Guadalajara (a major nearby city), and hosts a diverse fish population and migratory birds. More importantly, Lake Chapala serves as an environmental indicator as it sits inside the larger Lerma River system. Therefore, problems in Lake Chapala are indicators of the health of the entire ecosystem. For a very long time, the lake has been going through dehydration affected mainly by evaporation, increasing irrigation demands, drainage by the Santiago River, and water being piped to Guadalajara. As the lake level drops, the water has also become ``cloudy'', which matters because it affects how light and nutrients move through the lake water ecosystem.

One of the most visible and disruptive symptoms of these changes is recurring algal blooms. Even when blooms are not toxic, they still create real problems. They can be a nuisance for tourists and nearby communities, as blooms and associated plant growth (like seaweed and hyacinth) that interfere with motorized fishing boats, reports describe \cite{harwood-algae}. Algal blooms have also been linked to unpleasant taste and odor in drinking water for people in Guadalajara, requiring additional treatment.
...
Most importantly, when the population reaches zero, the derivative may still be negative. This would cause the solution to move to negative population values, which are not biologically meaningful. Thus, model~\ref{V1} does not satisfy the requirement that the population remain nonnegative.
...
To address the aforementioned limitations of model~\ref{V1}, particularly the issue of negative growth at zero population and the lack of feasibility, we modify the oscillating growth function by shifting it upward. The revised model now becomes
\begin{equation}
    \frac{dy}{dt} = 1- \cos(t)
    \label{V2}
    \tag{V2}
...
This upward shift is helpful because it prevents the model from forcing the population to decrease when the population is already small. As observed in model~\ref{V1}, the derivatives oscillated between $-1$ and $+1$, meaning the population could be pushed into negative values. In model~\ref{V2}, however, the derivative is always nonnegative.

Below, we plot the differential equation model~\ref{V2} on a slope field plotter and examine its features.
...
We plot the differential equation model~\ref{V3} on a slope field plotter below and observe how population $y$ changes over time $t$.
...
Below, we plot model~\ref{V4} on a slope field plotter to observe the changes due to introducing the death function.
...
We determine suitable values for the parameters $a$, $b$, and $h$ from the general model~\ref{2} using the following observed data. The goal is to select parameter values that reflect the observed spraying frequency, maximum rate of change, and subsequent decrease in growth over time.
...
\textbf{Parameter $b$:} Since spraying occurs every 15 days, the growth cycle has a period $T = 15$ days. We also know that the cosine function repeats after every $2\pi$ radians. Therefore, for one full cycle to occur in our model, the input of the cosine function increases by $2\pi$ after one period $T$.
...
\textbf{Parameters $a$ and $h$:} The general model~\ref{2} is
\begin{equation}
    \frac{dy}{dt} = a(1 - \cos(bt))y - hy.
\end{equation}
...
We solve the differential equation model~\ref{2} to obtain $y(t)$, the algae population density as a function of time $t$.

\subsection{General solution}

Assuming $h \ne a$ and applying the initial condition $y(0)=y_0$, we solve the general model. Since this differential equation is linear in $y$ (and therefore separable), we solve it using separation of variables.
...
Plugging in the initial condition $y(0)=y_0$ in the general solution~(\ref{3}), we get $C=y_0$. Therefore, the solution is
\begin{equation}
...
Substituting the parameter values computed in Section~\ref{sec:calibration} and the initial population density,
\[
y(0) = 81\, g/m^2,
\]
into~(\ref{4}), we write the two particular solutions:
...
Since both functions~(\ref{5}) and (\ref{5'}) generate nearly identical graphs while preserving all desired qualitative features of the model, it suffices to graph only one. Therefore, without loss of generality, we graph $y'(t)$ below.

\begin{figure}[h]
    \centering
    \includegraphics[width=0.65\linewidth]{figures/y-graph.png}
    \captionsetup{width=0.65\linewidth}
    \caption{Graph of the particular solution $y'(t)$ with initial population density $y(0) = 81~\mathrm{g/m^2}$ and calibrated parameter values.}
    \label{fig:population}
\end{figure}

Assuming population densities above $100~\mathrm{g/m^2}$ trigger offensive odors in the drinking water, we estimate the length of time intervals during which
\[
y'(t) > 100.
\]
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.65\linewidth]{figures/y-with-threshold.png}
    \captionsetup{width=0.65\linewidth}
    \caption{Graph of the particular solution $y'(t)$ with population density threshold $y(t) = 100~\mathrm{g/m^2}$.}
    \label{fig:threshold}
\end{figure}

\begin{enumerate}
    \item 1st interval: $[9, 14]$, (length = $5$ days)
    \item 2nd interval: $[24, 29]$, (length = $5$ days)
    \item 3rd interval: $[39, 43]$, (length = $4$ days)
    \item 4th interval: $[55, 58]$, (length = $3$ days)
    \item 5th interval: $[70, 72]$, (length = $2$ days)
\end{enumerate}

We observe that the exceeding intervals become progressively shorter over time due to the negative net growth, $a - h = -0.005$. The last time the population exceeds $100$ g/m$^2$ is approximately at $t = 72$ days. After that, even the peak amplitude stays below the odor threshold.

\section{Conclusion}

Throughout the paper, we developed a differential equation model that explains algal bloom pattern in Lake Chapala. The model starts with just a simple oscillatory growth idea and goes through systematic refinement to respect biological feasibility and realistic population behavior. Per the reference paper \cite{harwood-algae}, we built a general model that reflects both seasonal chemical influx (growth function) and consistent removal effects (death function), incorporating periodic growth and proportional death.
...
With initial population density of $81~\mathrm{g/m^2}$, the model predicts that the algae population exceeds the odor threshold of $100~\mathrm{g/m^2}$ during five early time intervals. However, these intervals shorten progressively, and the final exceedance occurs around day 72. After this point, the population remains below the threshold.

Overall, this mathematical model serves as a simplified yet informative representation of algal bloom dynamics in Lake Chapala. It demonstrates how differential equations can be used to model environmental systems and interpret real-world data.

\section*{Acknowledgments}
This research was conducted under the supervision of Professor Cayce Fylling (Gettysburg College). Environmental observations and scenario data are drawn from Harwood~\cite{harwood-algae}. Figures and supplementary materials are available at \url{https://github.com/soikat27/algal-bloom-de-model}.
